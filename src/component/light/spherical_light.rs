...
impl Light for SphericalLight {
    ...
    fn shading(&self, intersection: &Intersection, scene: &Scene) -> Color {
        ...
        let is_shadow = scene.compute_pixel(&shadow_ray).map_or(false, |intersect| intersect.fast_dist() > self.distance(contact) );
		let shadow_intensity = if is_shadow { 0.1 } else { 1.0 };
		
        let light_power = surface_normal.dot(direction_to_light).max(0.0) * self.intensity * shadow_intensity / decay;
